...
5 tools backed by LspClient + LanguageServerSymbolRetriever + CodeEditor.
...
class ReplaceSymbolBodyTool(BaseTool[ReplaceSymbolBodyArgs]):
    ...
    async def _do_apply(self, preview_id: str) -> ToolResult:
        ...
        try:
            result = self._editor.apply(preview_id)
...
def create_symbol_tools(
    ...
    """Return all 5 symbol tools ready for registry registration."""
    return [

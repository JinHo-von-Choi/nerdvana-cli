"""Live smoke for fireworks provider — invoked only when API key env is set."""
...
@pytest.mark.live
def test_smoke_fireworks() -> None:
    """Round-trip a 'reply with OK' prompt and verify a non-empty bounded response."""

    async def _call() -> None:
        provider = create_provider(provider="fireworks", max_tokens=MAX_LIVE_TOKENS)

"""Live smoke for featherless provider — invoked only when API key env is set."""
...
@pytest.mark.live
def test_smoke_featherless() -> None:
    """Round-trip a 'reply with OK' prompt and verify a non-empty bounded response."""

    async def _call() -> None:
        provider = create_provider(provider="featherless", max_tokens=MAX_LIVE_TOKENS)

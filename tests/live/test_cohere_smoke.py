"""Live smoke for cohere provider — invoked only when API key env is set."""
...
@pytest.mark.live
def test_smoke_cohere() -> None:
    """Round-trip a 'reply with OK' prompt and verify a non-empty bounded response."""

    async def _call() -> None:
        provider = create_provider(provider="cohere", max_tokens=MAX_LIVE_TOKENS)

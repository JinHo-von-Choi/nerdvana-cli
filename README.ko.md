...
AI 기반 CLI 개발 도구 — Anthropic Claude, OpenAI, Google Gemini, Groq, Ollama 등 **13개 AI 플랫폼**을 지원합니다.

## 기능

- **다중 제공자 지원** — 하나의 CLI로 13개 AI 플랫폼 사용 가능
...
| `/clear` | 현재 대화 내용 지우기 |
| `/init` | 현재 디렉토리에 `NIRNA.md` 프로젝트 지침 파일 생성 |
| `/model` | 현재 모델 표시. `/model <이름>`으로 모델 변경 (제공자 자동 감지) |
| `/models` | 현재 제공자 API에서 사용 가능한 모델 목록 조회 |
| `/provider` | 현재 제공자 표시. `/provider <이름>`으로 제공자 추가/전환 |
| `/mcp` | 연결된 MCP 서버 상태 표시 |
| `/tokens` | 누적 토큰 사용량 및 컨텍스트 윈도우 사용률 표시 |
| `/skills` | 등록된 에이전트 스킬 목록 표시 |
| `/tools` | 활성화된 모든 내장 도구 및 MCP 도구 목록 표시 |
| `/update` | 최신 버전 확인 및 업데이트 설치 |
| `/quit` | REPL 종료 |
| `/exit` | REPL 종료 (`/quit` 별칭) |
| `/q` | REPL 종료 (단축 별칭) |

## 내장 도구

NerdVana CLI 0.3.0은 17개의 내장 도구를 제공합니다. LSP 도구는 해당 언어 서버 바이너리가 설치된 경우에만 자동 등록됩니다.
